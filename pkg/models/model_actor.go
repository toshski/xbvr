...
func QueryActors(r RequestActorList, enablePreload bool) ResponseActorList {
	...
	if r.MinSceneRating.OrElse(0) > 0 {
		tx = tx.Where("(select AVG(s.star_rating) from scene_cast sc join scenes s on s.id=sc.scene_id where sc.actor_id =actors.id and s.star_rating > 1.5 and is_hidden=0) >= ?", r.MinSceneRating.OrElse(0))
	}
	if r.MaxSceneRating.OrElse(5) < 5 {
		tx = tx.Where("(select AVG(s.star_rating) from scene_cast sc join scenes s on s.id=sc.scene_id where sc.actor_id =actors.id and s.star_rating > 1.5 and is_hidden=0) <= ?", r.MaxSceneRating.OrElse(50))
	}
	var sites []string
	...
	case "scene_rating_desc":
		tx = tx.
			Order("(select AVG(s.star_rating) from scene_cast sc join scenes s on s.id=sc.scene_id where sc.actor_id =actors.id and s.star_rating > 1.5  and is_hidden=0) desc, (select count(*) from scene_cast sc join scenes s on s.id=sc.scene_id where sc.actor_id =actors.id and s.star_rating > 1.5 and is_hidden=0) desc, (select count(*) from scene_cast sc join scenes s on s.id=sc.scene_id where sc.actor_id =actors.id) desc")
	...
	(select AVG(s.star_rating) scene_avg from scene_cast sc join scenes s on s.id=sc.scene_id where sc.actor_id =actors.id and s.star_rating > 1.5 and is_hidden=0) as scene_rating_average	
	`)
